...
## Important: CLI Binary Reference

Akash uses **two separate CLI binaries** for different operations:

| Binary | Purpose | Used For |
|--------|---------|----------|
| **`akash`** | Node binary | Validators, full nodes, chain queries (`akash query staking`, `akash query gov`, `akash status`), governance, key management |
| **`provider-services`** | Tenant/provider CLI | Deployments (`tx deployment`), leases (`tx market`), certificates (`tx cert`), manifests (`provider send-manifest`), provider operations |

> **All deployment examples in this documentation use `provider-services`**, not `akash`.
...
# Accept bid
provider-services tx market lease create --dseq <dseq> --provider <provider> --from wallet
```
