...
Sources: Local repo analysis, browserpod.io docs, pricing page, CheerpX docs, Leaning Technologies ecosystem

---

## Executive Summary

BrowserPod is a proprietary SaaS product by Leaning Technologies that runs full language runtimes (currently Node.js v22) compiled to WebAssembly inside a browser tab. It uses CheerpX (x86-to-WebAssembly virtualization engine) and CheerpOS (Linux syscall layer) under the hood. It is browser-only — no headless, no server-side, no Node.js programmatic API. The npm package is a thin 16-line CDN loader. It cannot take screenshots natively. It can run Node.js test frameworks but not browser DOM tests. Portal URLs are publicly accessible and ephemeral.

---

## 1. Documentation — Key Pages

### Architecture Overview
Source: https://browserpod.io/docs/overview

4-layer architecture:
1. Runtime Loading — Language runtimes (Node.js v22) compiled to WebAssembly, targeting Linux syscall interface
2. Execution — Browser JS engine executes Wasm via WebWorkers for multi-threading and process isolation
3. Resource Virtualization — Block-based streaming virtual filesystem (POSIX-compatible), disk images streamed on-demand, changes stay local
4. Networking via Portals — When service binds to a port, BrowserPod creates a Portal (public URL routing traffic to the browser process)

Roadmap: Node.js v22 (now) → CLI tools, Python, Ruby, Go, Rust (2026) → Linux-class workloads (future)

Company: Leaning Technologies (leaningtech.com) — also builds Cheerp, CheerpJ, CheerpX, WebVM

### Quickstart Guide
Source: https://browserpod.io/docs/getting-started/quickstart

1. Register at console.browserpod.io (GitHub OAuth)
2. Create API key
3. `npm create browserpod-quickstart`
4. Enter project name + API key
5. `cd <project> && npm install && npm run dev`
6. Open localhost (e.g., http://localhost:5173/)
7. Modify src/main.js with hot reload

### Express.js Tutorial
Source: https://browserpod.io/docs/getting-started/expressjs

Pattern: Vite host project serves the page; inner project in `public/project/` runs inside BrowserPod.

Critical: vite.config.js MUST set COEP/COOP headers:
~~~js
...
static async boot(opts: {
  apiKey: string;
  nodeVersion?: string;     // Currently only "22"
  storageKey?: string;      // Multiple independent disks
}): Promise<BrowserPod>
~~~
Deducts 10 tokens from account on each call. `storageKey` enables multiple independent virtual disks.
...
~~~
Callback invoked when a Portal is created (service binds to a port).
...
~~~
REQUIRES a DOM element — browser-only, not headless-capable.

Full TypeScript definitions from local repo (`src/index.d.ts`):
~~~typescript
...
- Lives inside Pod runtime, NOT host machine's filesystem
- Persistence via IndexedDB, scoped to origin
...
- Processes see filesystem as local disk
...
Cross-Origin-Embedder-Policy: require-corp
~~~
Without these headers, browser blocks SharedArrayBuffer and pod fails to start.
Localhost exception: works over HTTP without headers.
Production requires HTTPS.

### Demos
Source: https://browserpod.io/docs/demos

- **PackagePod**: Live BrowserPod demo
...
## 3. CheerpX Relationship

### What is CheerpX?

Sources: https://cheerpx.io, https://leaningtech.com/cheerpx, https://labs.leaningtech.com/blog/cx-10

CheerpX is a WebAssembly-based x86 virtualization engine by Leaning Technologies. It runs unmodified x86 Linux binaries in the browser using a two-tier emulation approach (interpreter + JIT compiler). v1.0 stable released December 2024.
...
### Leaning Technologies Product Ecosystem
...
- https://github.com/leaningtech/svelte-browserpod-editor

---

## 4. Headless / Server-Side Use

**No. BrowserPod cannot run headless or server-side.**

Evidence:
- `createDefaultTerminal(consoleDiv: HTMLElement)` requires a DOM element
- Entire runtime depends on SharedArrayBuffer (browser-only API)
- SDK loaded from CDN via browser `import()` — no Node.js runtime loading
- npm package contains explicit SSR fallback returning null
- The npm package source (local repo src/index.js):
~~~js
const version = "2.0.2";
...
1. Open the Portal URL in a separate browser tab
2. Use external tools (Puppeteer, Playwright) to screenshot that URL
3. Or use browser-native `html2canvas` / canvas APIs on the page containing the Portal iframe
...
- Test code runs in Node.js context (server-side), not browser DOM context
- No built-in test runner or test discovery mechanism
- Test output appears in the Xterm.js terminal
- Must manually copy test files into pod filesystem and run them

For browser testing (DOM interactions), BrowserPod is NOT suitable. It runs Node.js, not a browser rendering engine. Use Playwright, Puppeteer, or Cypress instead.
...
3. Automatically creates a Portal — a secure, shareable public URL
4. The `onPortal` callback fires with `{ url, port }`
5. URL can be loaded in an iframe, new tab, or shared externally
...
- URL routes traffic from the internet through BrowserPod's infrastructure to your browser tab
- Works even though the server runs client-side

### Technical Constraints

- Portal URLs are ephemeral — tied to the browser session
- If the browser tab closes, the Portal becomes unreachable
- Performance depends on browser's resources and network connection
- Portal routing adds latency compared to direct server access

---

## 8. Language Runtime Support
...
## 9. Limitations, Performance, and Security
...
9. No outbound networking — On free/pro tiers; enterprise only with whitelists
...
- No usage analytics on free/pro tiers (enterprise only has admin & analytics API)
...
### npm package (browserpod v2.0.2)

The npm package is a thin wrapper (16 lines of JavaScript) that dynamically imports the actual runtime from CDN.
...
Proprietary. Copyright 2025 Leaning Technologies. Free for personal/open-source only. Commercial use requires paid plan.
...
| v2.0.0 | CLI tools (git, bash, curl, grep, openssl, vi), 2GB /home mount, storageKey for multiple disks |
...
| v1.1.0 | Major framework support (Next.js, Nuxt, Express, Svelte, React), HTTP compression via portals |
...
- `/a0/usr/workdir/browserpod-research-web.md` (645 lines) — full web research output
