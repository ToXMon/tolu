...
| GET | `/publish/:slug` | Required | Get site details + manifest |
...
| Method | Endpoint | Auth | Description |
|--------|----------|------|-------------|
| POST | `/auth/agent/request-code` | None | Request email verification code |
| POST | `/auth/agent/verify-code` | None | Verify code, receive API key |

## Aliases
- All `/publish` → `/artifact` also works
- `/publishes` → `/artifacts` also works
