...
Production-grade autonomous coding system for Agent Zero. Combines Ralph PRD-driven loops, 0xSero workflow principles, Ryan Carson's Code Factory and reflection patterns, Karpathy coding discipline, and vibe coding task framing.
...
cp -r /a0/usr/workdir/tolu/skills/agentic-coding-harness/* "$SKILL_DIR/"
...
| Path | Purpose | Lines |
|---|---|---|
| `SKILL.md` | Main skill definition with triggers | ~210 |
| `README.md` | This file — installation & reference | ~200 |
...
| `instruments/reflection_logger.py` | Generate session handoff and reflection docs | ~282 |
...
| `skills/skill.reflection.md` | `/reflect` — session reflection + handoff | ~174 |
...
| `tasks/code_factory.md` | `/code-factory` — auto-write + auto-review pipeline | ~226 |
| **Knowledge** | **Reference documentation** | |
| `knowledge/0xsero_coding_standards.md` | 0xSero coding standards reference | ~268 |
| `knowledge/agentic_coding_standards.md` | Combined 0xSero + Ryan Carson reference | ~449 |
| **Prompts** | **System prompt addons** | |
| `prompts/agent.system.addon.agentic-harness.md` | Behavioral defaults | ~110 |
| **Templates** | **File templates** | |
| `templates/prd.json.example` | Ralph-format PRD template with risk tiers | ~85 |
...
| `/reflect` | Session reflection: distill learnings, update progress.txt, generate handoff document |
| `/code-factory` | Run Code Factory pipeline: auto-write, risk gate, CI, review, remediate, merge |
...
11. **Reflect before closing** — Every session ends with `/reflect`. Distill learnings into progress.txt and AGENTS.md for the next session.

12. **Risk-tiered review** — Code Factory gates match change risk level. Critical changes get full human review; low-risk changes auto-merge.
...
Before closing any session, run `/reflect`. The model distills:
- What was accomplished
- What surprised (unexpected behaviors, API changes, dependency issues)
- What blocked progress
- What patterns were discovered
- What the next session should tackle

This becomes a handoff document at `work_logs/reflection_YYYY-MM-DD_HHMMSS.md` and accumulates in `progress.txt`.
...
2. **Risk Policy Gate** classifies change tier:
   - **Critical**: auth, payments, security, data migration → full human review
   - **High**: API routes, business logic, DB queries → automated + human review
   - **Medium**: UI components, configs, docs → automated review only
   - **Low**: typos, comments, formatting → auto-merge
3. **CI fanout** runs tier-appropriate checks
4. **Review agent** validates with `/audit`
5. **Remediation agent** fixes issues (max 3 cycles)
6. **Merge** based on tier policy
...
# Code Factory (alternative)
/code-factory                    → Run auto-write + auto-review pipeline

# Day end
/reflect                         → Distill learnings, generate handoff document
/end                              → Log session, update AGENTS.md
```
...
- [AI Dev Tasks](https://github.com/snarktank/ai-dev-tasks) — Ryan Carson's 3-file workflow
...
- [0xSero](https://x.com/0xSero) — Modular slice architecture and workpack patterns
